...
var showWork=(function(config){
    ...
    function getAllWork(showPagination){
        ...
        if(stage>=8){
            ...
            param.searchTopic=undefined;
            delete param.searchTopic;
        }
        $.ajax({
            url:config.ajaxUrls.getAllWorkJudge,
            type:"get",
            dataType:"json",
            data:param,
    ...
    return {
        ...
        showWorkDetail:function(el){
            ...
            $("#blackout").removeClass("hidden").css("zIndex","9999");
            ...
            $("#workInfo").load(el.attr("href")+" #workInfo",function(){
                ...
                if(location.href.indexOf("_lang=en")!=-1){
                    prevWork.attr("href","s/user/artifact/show/"+prevWorkId+"?_lang=en");
                    nextWork.attr("href","s/user/artifact/show/"+nextWorkId+"?_lang=en");
                }else{
                ...
                $.ajax({
                    ...
                    success:function(data){
                        if(data.success){
                            ...
                            //设置已经打了的数量
                            if(scoreObj[id]==-1){
                                var oldScoreCount=parseInt($("#countScored").text());
                                $("#countScored").text(oldScoreCount+1);
                            }
        ...
        searchByScored:function(el){
            if(el.hasClass("active")){
                searchIfScore="";
                el.removeClass("active");
                $(".queryItem:eq(0)").addClass("active");
            }else{
                searchIfScore=el.data("is-score");
                $(".queryItem").removeClass("active");
                el.addClass("active");
            }
